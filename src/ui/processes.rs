...
pub fn render_processes_tab(f: &mut Frame, app: &App, area: Rect) {
    ...
    let filter_text = if app.is_searching {
        format!(
            "🔍 Search Filter (type to filter, Enter/Esc to lock): {}_",
            app.search_query
        )
    } else if !app.search_query.is_empty() {
        format!(
            "🔍 Filter Active: '{}' (Press '/' to edit, Esc to clear) | Sort: {:?}",
            app.search_query, app.sort_key
        )
    } else {
        format!(
            "⚙️ Total Processes: {} | Sort: {:?} (Press 's' to cycle sort, '/' to search, 'k' to kill)",
            app.metrics.processes.len(),
            app.sort_key
        )
    };
    ...
    let search_bar = Paragraph::new(filter_text)
        ...
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title(" Process Manager Controls "),
        );
    ...
    .block(
        Block::default()
            .borders(Borders::ALL)
            .title(format!(" Processes ({}) ", app.metrics.processes.len())),
    );
